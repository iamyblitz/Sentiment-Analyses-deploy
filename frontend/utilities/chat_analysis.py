...
def chat_analysis(backend_url):
        ...
        if uploaded_file is not None:
            with st.spinner("Идет анализ..."):
                try:
                    ...
                    # Активность и настроение по времени
    ...
    # Если переменная df создана (анализ прошёл успешно), добавляем кнопку для скачивания CSV
    if df is not None:
        df_filtered = df.drop(columns=["datetime"], errors="ignore")
        csv_buffer = io.StringIO()
        df_filtered.to_csv(csv_buffer, index=False, encoding="utf-8")
        csv_bytes = csv_buffer.getvalue().encode()
        st.sidebar.header("Размеченный DataFrame в формате CSV")
